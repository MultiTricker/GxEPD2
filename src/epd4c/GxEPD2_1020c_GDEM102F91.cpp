...
// https://www.good-display.com/product/467.html
// Panel: GDEM102F91 : https://www.good-display.com/product/467.html
// Controller: ?, no specs, see GDEM102F91 panel specs for command list subset
//
...
#include "GxEPD2_1020c_GDEM102F91.h"

GxEPD2_1020c_GDEM102F91::GxEPD2_1020c_GDEM102F91(int16_t cs, int16_t dc,
...
void GxEPD2_1020c_GDEM102F91::clearScreen(uint8_t value) {
  clearScreen(value, 0xFF);
}

void GxEPD2_1020c_GDEM102F91::clearScreen(uint8_t black_value,
                                           uint8_t color_value) {
  writeScreenBuffer(black_value, color_value);
  refresh();
}

void GxEPD2_1020c_GDEM102F91::writeScreenBuffer(uint8_t value) {
  writeScreenBuffer(value, 0xFF);
}

void GxEPD2_1020c_GDEM102F91::writeScreenBuffer(uint8_t black_value,
                                                 uint8_t color_value) {
...
void GxEPD2_1020c_GDEM102F91::writeImage(const uint8_t bitmap[], int16_t x,
  ...
  if (hasPartialUpdate) {
    ...
    for (int16_t i = 0; i < h1; i++) {
      for (int16_t j = 0; j < w1 / 8; j++) {
        ...
        uint32_t idx = mirror_y ? j + dx / 8 + uint32_t((h - 1 - (i + dy))) * wb
                                : j + dx / 8 + uint32_t(i + dy) * wb;
        if (pgm) {
...
void GxEPD2_1020c_GDEM102F91::writeImage(const uint8_t *black,
...
void GxEPD2_1020c_GDEM102F91::writeImagePart(const uint8_t bitmap[],
    ...
    for (int16_t i = 0; i < h1; i++) {
      for (int16_t j = 0; j < w1 / 8; j++) {
        ...
        uint32_t idx =
            mirror_y
                ? x_part / 8 + j + dx / 8 +
                      uint32_t((h_bitmap - 1 - (y_part + i + dy))) * wb_bitmap
                : x_part / 8 + j + dx / 8 +
                      uint32_t(y_part + i + dy) * wb_bitmap;
        if (pgm) {
#if defined(__AVR) || defined(ESP8266) || defined(ESP32)
          data = pgm_read_byte(&bitmap[idx]);
#else
        ...
        if ((j >= x1) && (j < x1 + w) && (i >= y1) && (i < y1 + h)) {
          ...
          uint32_t idx =
              mirror_y
                  ...
                  : (x_part + j - x1) / 8 +
                        uint32_t(y_part + i - y1) * wb_bitmap;
          if (pgm) {
#if defined(__AVR) || defined(ESP8266) || defined(ESP32)
            data = pgm_read_byte(&bitmap[idx]);
#else
...
void GxEPD2_1020c_GDEM102F91::writeImagePart(
...
void GxEPD2_1020c_GDEM102F91::writeNative(const uint8_t *data1,
  ...
  if (hasPartialUpdate) {
    ...
    for (int16_t i = 0; i < h1; i++) {
      for (int16_t j = 0; j < w1 / 4; j++) {
        uint8_t data;
        // use wb, h of bitmap for index!
        uint32_t idx = mirror_y ? j + dx / 4 + uint32_t((h - 1 - (i + dy))) * wb
                                : j + dx / 4 + uint32_t(i + dy) * wb;
        if (pgm) {
#if defined(__AVR) || defined(ESP8266) || defined(ESP32)
      ...
      for (int16_t i = 0; i < int16_t(HEIGHT); i++) {
        for (int16_t j = 0; j < int16_t(WIDTH); j += 4) {
          ...
          if (data1) {
            if ((j >= x) && (j < x + w) && (i >= y) && (i < y + h)) {
              uint32_t idx =
                  mirror_y ? (j - x) / 4 + uint32_t((h - 1 - (i - y))) * wb
...
void GxEPD2_1020c_GDEM102F91::writeNativePart(
...
void GxEPD2_1020c_GDEM102F91::drawImage(const uint8_t bitmap[], int16_t x,
                                         int16_t y, int16_t w, int16_t h,
                                         bool invert, bool mirror_y, bool pgm) {
...
void GxEPD2_1020c_GDEM102F91::drawImagePart(const uint8_t bitmap[],
                                             int16_t x_part, int16_t y_part,
                                             int16_t w_bitmap, int16_t h_bitmap,
                                             int16_t x, int16_t y, int16_t w,
                                             int16_t h, bool invert,
...
void GxEPD2_1020c_GDEM102F91::drawImage(const uint8_t *black,
                                         const uint8_t *color, int16_t x,
                                         int16_t y, int16_t w, int16_t h,
                                         bool invert, bool mirror_y, bool pgm) {
...
void GxEPD2_1020c_GDEM102F91::drawImagePart(
    const uint8_t *black, const uint8_t *color, int16_t x_part, int16_t y_part,
    int16_t w_bitmap, int16_t h_bitmap, int16_t x, int16_t y, int16_t w,
    int16_t h, bool invert, bool mirror_y, bool pgm) {
...
void GxEPD2_1020c_GDEM102F91::drawNative(const uint8_t *data1,
                                          const uint8_t *data2, int16_t x,
                                          int16_t y, int16_t w, int16_t h,
                                          bool invert, bool mirror_y, bool pgm) {
...
void GxEPD2_1020c_GDEM102F91::refresh(bool partial_update_mode) {
  if (hasPartialUpdate)
    _setPartialRamArea(0, 0, WIDTH, HEIGHT);
  _refresh(partial_update_mode);
}

void GxEPD2_1020c_GDEM102F91::refresh(int16_t x, int16_t y, int16_t w,
                                       int16_t h) {
...
void GxEPD2_1020c_GDEM102F91::powerOff() { _PowerOff(); }

void GxEPD2_1020c_GDEM102F91::hibernate() {
  _PowerOff();
  if (_rst >= 0) {
...
void GxEPD2_1020c_GDEM102F91::setPaged() {
  _paged = true;
  if (!_init_display_done)
    _InitDisplay();
  _writeCommand(0x10);
...
void GxEPD2_1020c_GDEM102F91::_refresh(bool partial_update_mode) {
...
void GxEPD2_1020c_GDEM102F91::_setPartialRamArea(uint16_t x, uint16_t y,
...
void GxEPD2_1020c_GDEM102F91::_PowerOn() {
  if (!_power_is_on) {
    _writeCommand(0x04);
...
void GxEPD2_1020c_GDEM102F91::_PowerOff() {
  if (_power_is_on) {
    _writeCommand(0x02);
    _writeData(0x00);
...
void GxEPD2_1020c_GDEM102F91::_InitDisplay() {
  if ((_rst >= 0) && (_hibernating || _initial_write)) {
    digitalWrite(_rst, HIGH);
    delay(20);               // At least 20ms delay
    digitalWrite(_rst, LOW); // Module reset
    delay(2);                // At least 10ms delay, 2ms for WS "clever" reset
    digitalWrite(_rst, HIGH);
    delay(2); // At least 10ms delay (measured)
...
uint8_t GxEPD2_1020c_GDEM102F91::_invert(uint8_t data) {
  uint8_t result = 0x00;
  for (int i = 0; i < 4; i++) {
    switch (data & 0x03) {
    case 0x00:
